...
무료 Render의 파일 시스템은 재시작·유휴 종료·재배포 때 초기화될 수 있습니다. 이 앱은 Supabase Storage의 비공개 버킷에 상태와 업로드 파일을 보관해 Render가 다시 시작해도 복원할 수 있습니다.
...
환경변수를 저장하고 Render에서 한 번 재배포하면 앱이 버킷의 `state.json`과 `files/` 객체를 사용합니다. 원격 저장소에 아직 상태가 없으면 첫 번째 관리자 저장 때 자동으로 생성됩니다.
...
이 명령은 기존 `storage/portfolio.pstore`와 업로드 파일을 원격 버킷에 한 번 올립니다. 이미 원격 `state.json`이 있으면 덮어쓰지 않고 중단합니다. 계정 비밀번호와 비공개 자료도 원격에 저장되므로 버킷을 공개로 바꾸지 마세요.
